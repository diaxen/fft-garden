	  ...
	  int log2_N)
{
  ...
  for (unsigned j = 2 - (log2_N & 1); j < log2_N; j += 2)
	...
	{
	  complex double t1 = get_twiddle(k << s, log2_N);
	  complex double t2 = get_twiddle(k * 2 << s, log2_N);
	  complex double t3 = get_twiddle_mr(k * 3 << s, log2_N);
          complex double *t = out;

	  for (unsigned i = 0; i < (1 << s); i++)
	    {
	      mr_dit_bf4(l, t + k, t1, t2, t3);
	      t += 1 << (j + 2);
	    }
	}
    }
